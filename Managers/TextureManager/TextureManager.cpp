...
void TextureManager::LoadTexture(const std::string& filePath) {
	...
	D3D12_CPU_DESCRIPTOR_HANDLE cpuHandle = GetCPUDescriptorHandle(dxCommon->GetSRVDescriptorHeap(), descriptorSize, static_cast<uint32_t>(textures_.size() + 1));
	D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle = GetGPUDescriptorHandle(dxCommon->GetSRVDescriptorHeap(), descriptorSize, static_cast<uint32_t>(textures_.size() + 1));
